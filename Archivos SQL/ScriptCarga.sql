...
BEGIN
	SELECT id_cliente INTO idCliente FROM Cliente WHERE nombre = _nombre AND getIdPais(_pais) = (SELECT id_pais FROM Pais WHERE pais=_pais);
	RETURN idCliente;
END;
$$  LANGUAGE plpgsql;
...
INSERT INTO Poliza    (id_poliza, id_est, id_condp, fecha_inicio, fecha_fin, clausulas, id_cli, id_ts, poliza_vieja, meses, producto_descripcion,
						status, precio, valor_seguro, id_vendedor, id_operador, id_negociador, cod_venta, coberturas_adicionales, id_cargaV, id_cargaO, id_cargaN)
SELECT DISTINCT id, getIdEstado(estado), 1, fecha_inicio, fecha_fin, '', getIdCliente(cliente, pais), getIdTS(tipo), poliza_vieja, meses, producto_desc,
				estado, precio, valor_seguro, getIdEmp('Vendedor', codigo_vendedor, codigo_oficina, getIdDepartamento(depto)), getIdEmp('Operador', codigo_operador, codigo_oficina, getIdDepartamento(depto)),
...
SELECT DISTINCT 'Cobertura 1', cob1, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 2', cob2, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 3', cob3, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 4', cob4, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 5', cob5, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 6', cob6, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 7', cob7, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 8', cob8, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 9', cob9, getIdTS(tipo), id
FROM Datos;

INSERT INTO Cobertura (descripcion, monto, id_ts, id_poliza)
SELECT DISTINCT 'Cobertura 10', cob10, getIdTS(tipo), id
FROM Datos;
...
INSERT INTO Plan_De_Pagos(prima, numero_cuotas, recargo, id_moneda, numero_cuotas_atrasadas, id_ts, id_poliza, porcentaje_deducible,
							deducible, tipo_cuota, id_tp)
SELECT DISTINCT precio/cuotas, cuotas, tasa, getIdMoneda(moneda), cuotas_atrasadas, getIdTS(tipo), id, pct_deducible, deducible, 'Nivelada',getIdTipoPago(tipo_pago)
FROM Datos;
